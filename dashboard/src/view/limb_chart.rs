...
use common::param::*;
use ratatui::{layout::Rect, style::{Color, Style, Stylize}, symbols, widgets::{Axis, Block, Chart, Dataset, GraphType}, Frame};

use crate::model::{self, Limb, Model};

pub fn render(frame: &mut Frame, rect: Rect, model: &Arc<Mutex<Model>>, pc: &mut ParameterController) {
    let (body, bmin, bmax) = model.lock().unwrap().dataset(model::LimbDataset::Rms, Limb::Body);
    let (mouth, mmin, mmax) = model.lock().unwrap().dataset(model::LimbDataset::Rms, Limb::Mouth);
