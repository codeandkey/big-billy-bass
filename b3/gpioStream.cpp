...
void GpioStream::onStart() {
    ...
    gpioSetMode(PIN_HEAD, PI_OUTPUT);
    gpioSetMode(PIN_TAIL, PI_OUTPUT);
    gpioSetMode(PIN_MOUTH, PI_OUTPUT);
}
...
void GpioStream::_applyGpioFrame(const GpioFrame& data) {
    ...
    // TODO : actual algorithm for switching tail/head movement
    gpioPWM(PIN_MOUTH, data.hpf);
    gpioPWM(PIN_TAIL, data.lpf);
}
...
int GpioChunk::_extractGpioFrame(uint64_t now, const GpioStreamConfig& config,
    ...
    int window_width = config.gpio_window * config.sample_rate / 1000; // TODO: moveme to const
    int window_low = max(0, sample_index - window_width / 2);
    int window_high =
        min((int)lpf.size(), sample_index + window_width / 2);
